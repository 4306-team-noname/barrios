...
class UploadService():
    ...
    async def get_upload_by_id(self, id: int) -> Result:
        ...
        if upload_result['id'] == None:
            if upload_result['errors'] != None:
    ...
    async def save_file(self, file) -> Result:
        ...
        if not self.is_valid_filetype(ext):
            return {'ok': False, 'error': f'{ext} is not a valid file type', 'value': None}
    ...
    def delete_file(self, file_path) -> Result:
        try:
            ...
            return {'ok': False, 'error': f'No file to delete at {file_path}', 'value': None}
    ...
    async def create_upload(self, file) -> Result:
        ...
        return {'ok': True, 'value': {'id': upload['id'], 'file_name': file.filename, 'file_location': file_location}, 'error': None}