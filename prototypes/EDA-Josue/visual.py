...
category_df = df_category[df_category['category_name'] == 'ACY Inserts']
...
result_df = pd.DataFrame({'Week': range(1, total_duration + 1), 'Usage Rate': [average_weekly_usage_rate] * total_duration})

# Plotting
plt.figure(figsize=(10, 6))
plt.plot(result_df['Week'], result_df['Usage Rate'], marker='o', color='blue', label='Average Weekly Usage Rate')
plt.title('Average Weekly Usage Rate for Category KTO')
plt.xlabel('Week')
plt.ylabel('Usage Rate')
plt.xticks(result_df['Week'])
plt.legend()
plt.grid(True)
plt.show()