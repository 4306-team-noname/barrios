...
def get_dock_days(flight_plan: DataFrame):
    ...
    dates_list = [
        dt.datetime.strptime(date, "%m/%d/%Y").date() for date in dock_days_list
    ]

    # returns a list of the days difference between each date
    return (
        pd.Series(dates_list).diff().dt.days.iloc[1:].astype(int).tolist(),
        dock_days_list,
        (len(dock_days_list) - 1),
    )
...
def consumable_ascension(flight_plan: DataFrame, rater: DataFrame, consumable):
    ...
    for i in range(0, len(listofrates)):
        res_list.append(
            ((listofrates[i] + listofrates_crew[i]) - sum_generated) * num_days[i]
        )
